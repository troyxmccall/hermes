...
func bundle(userYamlPath, outputPath string) {
	...
	hermesPath, err := os.Executable()
	checkError(err, "Could not find path to hermes")

	archive := NewArchive(archivePath)

	for _, path := range []string{userYamlPath, hermesPath} {
	...
	execute := `#!/bin/bash
set -eu
export TMPDIR=$(mktemp -d /tmp/hermes.XXXXXX)
ARCHIVE=$(awk '/^__HERMES_ARCHIVE__/ {print NR + 1; exit 0; }' $0)
...
func storeSudoPasswd() {
	...
	if requiresPassword {
		fmt.Print("[hermes] sudo password required: ")
...
func main() {
	...
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:        "cache-path",
			Value:       "",
			Usage:       "The path where cached files will be stored. By default '$(pwd)/.hermes' is used.",
	...
	app.Commands = []cli.Command{
		{
			...
			Action: func(cliCtx *cli.Context) error {
				if cliCtx.NArg() < 1 {
					exitWithErrorMessage("Must provide the path to a hermes yaml file")
				} else if cliCtx.NArg() > 1 {
					exitWithErrorMessage("Only one hermes yaml file can be provided at a time")
				}
		...
		{
			Name:  "run",
			Usage: "Execute the given yaml file with hermes",
			Flags: []cli.Flag{
				cli.StringFlag{
					Name:  "tags",
					Value: "",
