...
type TaskCommand struct {
	...
	// EnvReadFile is an extra pipe given to the child shell process for exfiltrating env vars back up to hermes (to provide as input for future tasks)
	EnvReadFile *os.File
...
func (task *Task) inflateCmd() {
	...
	task.Command.Cmd = exec.Command(shell, "-c", sudoCmd+task.Config.CmdString+"; HERMES_RC=$?; env >&3; exit $HERMES_RC")
	...
	if (len(task.Command.Cmd.Dir) == 0 && len(task.Config.ParentCwdString) != 0){
	...
	} else if (len(task.Command.Cmd.Dir) != 0) {
		LogToMain("CWD was set at the task level for "+task.Config.Name, infoFormat)
	} else {
