...
// CliOptions is the exhaustive set of all command line options available on hermes
...
type OptionsConfig struct {
	...
	// Bundle is a list of relative file paths that should be included in a hermes bundle
	...
	// SingleLineDisplay indicates to show all hermes output in a single line (instead of a line per task + a summary line)
...
func readTimeCache() {
	if Config.CachePath == "" {
		...
		Config.CachePath = path.Join(cwd, ".hermes")
	}
